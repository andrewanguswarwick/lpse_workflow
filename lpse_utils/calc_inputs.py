...
# 1D backscattered SRS LW frequency calculation
def bsrs_lw_envelope(case):
...
def spectral_dt(case,freqs,D=1,dt_frac=0.95):
  ...
  for i in range(waves):
    dts[i] = 8*freqs[i]*deltax**2/(lambda0*c*D*(1-AA)**2)
  ...
  for i in case.setup_classes:
    if isinstance(i,wf.light_control):
      ...
      print(f'Laser spectral dt is: {dts[0]:0.2e}')
      print(f'Raman spectral dt is: {dts[1]:0.2e}')
    elif isinstance(i,wf.lw_control):
      i.lw.spectral.dt = dts[2]
      print(f'LW spectral dt is: {dts[2]:0.2e}')
    elif isinstance(i,wf.iaw_control):
      i.iaw.spectral.dt = dts[3]
      print(f'IAW spectral dt is: {dts[3]:0.2e}')
